...
fn main() {
    ...
    let mut runtime = crepe::Runtime::new();
    runtime.extend(&[
        Volume(Box::leak(volume.clone().into_boxed_str())),
        Workload(Box::leak(workload.clone().into_boxed_str())),
        SLA(Box::leak(sla.clone().into_boxed_str())),
        Budget(Box::leak(budget.clone().into_boxed_str())),
        Observability(Box::leak(observability.clone().into_boxed_str())),
    ]);

    let (recs, expls) = runtime.run();

    let mut recs_adjusted: Vec<_> = recs.into_iter().map(|Recommendation(r, base)| {
